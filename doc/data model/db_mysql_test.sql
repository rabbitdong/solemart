...
CREATE INDEX idx_email_users ON TestUserItems(Email);
ALTER TABLE TestUserItems ADD CONSTRAINT uni_username UNIQUE (UserName);

#匿名账户的用户ID(UserID=1固定为匿名用户)
INSERT INTO TestUserItems VALUES(1, 'Anonymous', '', '', 0, '0001-01-01 00:00:00', 1, '');

#User Append Info
CREATE TABLE TestUserAppendInfoItems(
	UserID 		INTEGER PRIMARY KEY REFERENCES TestUsers(UserID),
	NickName	VARCHAR(30),
...
CREATE TABLE TestUserPointItems(
...
CREATE TABLE TestCategoryItems(
	CategoryID			INTEGER AUTO_INCREMENT PRIMARY KEY, #类别ID
	CategoryName		NVARCHAR(10) NOT NULL,
	Description			NVARCHAR(100),			#类别的描述
	ParentCategoryID	INTEGER NULL REFERENCES TestCategoryItems(CategoryID)			#该类别的父类别ID
) CHARACTER SET utf8;

#Brand of production
CREATE TABLE TestBrandItems(
...
INSERT INTO TestBrandItems(ZhName, EnName, Description) VALUES(N'杂牌', N'nobrand', N'无正规的知名品牌，或贴牌商品');

#The vendor infomation
CREATE TABLE TestVendorItems(
...
CREATE TABLE TestProductItems(
	ProductID		INTEGER AUTO_INCREMENT PRIMARY KEY,
	CategoryID		INTEGER REFERENCES TestCategoryItems(CategoryID),
...
CREATE TABLE TestSaledProductItems(
	ProductID	INTEGER PRIMARY KEY REFERENCES TestProductItems(ProductID),
...
#商品的图像信息表
CREATE TABLE TestProductImageItems(
	ImageID		INTEGER AUTO_INCREMENT PRIMARY KEY ,
	ProductID	INTEGER REFERENCES TestProductItems(ProductID) ON DELETE CASCADE,
	MimeType	VARCHAR(20),
...
#商品的评价表
CREATE TABLE TestProductCommentItems(
...
CREATE TABLE TestPriceHistoryItems(
...
CREATE TABLE TestSendAddressItems(
	UserID 		INTEGER PRIMARY KEY REFERENCES TestUserItems(UserID),
...
#system billboards.
CREATE TABLE TestBulletinItems(
...
#User favorite list.
CREATE TABLE TestFavoriteItems(
	UserID			INTEGER REFERENCES TestUserItems(UserID),
	ProductID		INTEGER REFERENCES TestProducts(ProductID),
	FavoriteTime		DATETIME,
	Description		NVARCHAR(200),
	CONSTRAINT pk_TestFavorites PRIMARY KEY (UserID, ProductID)
) CHARACTER SET utf8;
